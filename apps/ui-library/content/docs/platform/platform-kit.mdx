---
title: Platform Kit
description: The easiest way to build platforms on top of Selfbase
---

<video width="100%" muted autoPlay controls>
  <source
    src="https://xguihxuzqibwxjnimxev.selfbase.co/storage/v1/object/public/videos/marketing/blog/platform-kit/platform-kit.mp4"
    type="video/mp4"
  />
...
<BlockItem
  ...
  description="Embeddable dialog for managing Selfbase projects via the Management API."
/>
...
The Platform Kit is a collection of customizable API's, hooks and components you can use to provide an embedded Selfbase experience within your own platform. It comes in the form of a single dialog that enables the management of database, authentication, storage, users, secrets, logs, and performance monitoring.
...
import SelfbaseManagerDialog from '@/components/selfbase-manager'
...
export default function Example() {
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)}>Open Selfbase Manager</Button>
      <SelfbaseManagerDialog
...
2. **Add project-level authentication checks** in your API proxy at `app/api/selfbase-proxy/[...path]/route.ts` as well as your ai/sql route at `app/api/ai/sql/route.ts` to ensure only authorized users can access their own project resources.
...
That's it! The default setup uses your Selfbase personal access token for the Management API.
...
- [Selfbase Management API](https://selfbase.com/docs/reference/api/introduction)
